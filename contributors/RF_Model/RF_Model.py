...
# from sklearn import tree #Shahab added this
# from sklearn.tree import DecisionTreeClassifier #Shahab added this
# from sklearn import svm
from sklearn.ensemble import RandomForestRegressor
from joblib import dump #Shahab added this
from joblib import load #Shahab added
...
#         x = layers.Dense(1)(x)
        ...
        # model = keras.Model(inputs=input_1,outputs=x) #Shahab commented 
        ...
        # model.fit(X_train, y_train, epochs=epochs, batch_size=100,
                            # validation_data=(X_test,y_test),shuffle=True,callbacks=[callback], verbose=0) #Shahab commented
        model.fit(X_train, y_train)

        # Save the best model to a file
        dump(model, f"{checkpoint_filepath}best_rf_model.joblib")
        print("Shahab running the model")
...
def Model_predict(cwd, RegionTest, RegionObs_Test, RegionTest_notScaled):
    ...
    for Region in Regions:
     ...
     #Set up model to predict 0" SWE if hasSnow = False
        ...
        y_pred_VIIRS.reset_index(inplace = True)
        ...
        # bestmodel = checkpoint_filepath+bestmodel[2]
        # model=load_model(bestmodel)
        model=load(f"{checkpoint_filepath}/best_rf_model.joblib")
       # print(bestmodel)
        #save this model
        # model.save(f"{checkpoint_filepath}{Region}_model.keras")
        # model.dump(f"{checkpoint_filepath}{Region}_model.keras") #Shahab commented
        #make sure the model loads
        # model = keras.models.load_model(f"{checkpoint_filepath}{Region}_model.keras") #Shahab commented
        ...
        pred_obs.reset_index(inplace = True)
        X_test_notscaled.reset_index(inplace = True, drop = True)
        ...
        pred_obs.y_pred_fSCA = pred_obs.y_pred_fSCA.fillna(pred_obs.y_pred)
        ...
        print("Shahab running this")
    return Predictions, pred_obs, X_test_notscaled
